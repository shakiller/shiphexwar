class HexagonalBattleship {
    ...
    getShipPositions(row, col, size, orientation) {
        ...
        // Простые и понятные направления для гексагональной сетки
        const directions = [
            // Вертикаль вниз (↓)
            () => ({ dr: 1, dc: 0 }),
            
            // Диагональ вниз-вправо (↘) - ПРАВИЛЬНО: (3,1)(3,2)(2,3)(2,4)
            () => ({ dr: 0, dc: 1 }), // Четные шаги: вправо
            () => ({ dr: -1, dc: 1 }), // Нечетные шаги: вверх-вправо
            
            // Диагональ вниз-влево (↙)
            () => ({ dr: 0, dc: -1 }), // Четные шаги: влево
            () => ({ dr: -1, dc: 0 }), // Нечетные шаги: вверх
            ...
            () => ({ dr: 0, dc: -1 }), // Четные шаги: влево
            () => ({ dr: -1, dc: -1 }), // Нечетные шаги: вверх-влево
        ...
        // Для диагональных направлений используем чередование шагов
        const directionPairs = [
            [directions[0]], // Вертикаль вниз - всегда один шаг
            [directions[1], directions[2]], // ↘: чередование вправо/вверх-вправо
            [directions[3], directions[4]], // ↙: чередование влево/вверх
            [directions[5]], // Вертикаль вверх - всегда один шаг
            [directions[6], directions[7]], // ↖: чередование влево/вверх-влево
            [directions[8], directions[9]]  // ↗: чередование вправо/вверх
        ];
        
        const dirPair = directionPairs[orientation];
        
        for (let i = 1; i < size; i++) {
            // Для диагональных направлений чередуем шаги
            const stepIndex = dirPair.length > 1 ? (i - 1) % dirPair.length : 0;
            const dirFunc = dirPair[stepIndex];
            const dir = dirFunc();
